...
import { Actor } from '../engine/turnOrder';
...
describe('StatusManager', () => {
  ...
  test('バフ効果の追加・取得', () => {
    ...
    const result = sm.addEffectBySkill(ownerId, 'バイキルト');
    expect(result.applied).toBe(true);
    const effects = sm.getEffects(ownerId);
    expect(effects.length).toBe(1);
    expect(effects[0].effect).toBe('atk_up');
  });

  test('stackable:false のバフは重複不可', () => {
    const ownerId = 'test_actor';
    // バイキルトは stackable:false
    const first = sm.addEffectBySkill(ownerId, 'バイキルト');
    const second = sm.addEffectBySkill(ownerId, 'バイキルト');
  ...
  test('tickTurn でターン経過と効果消滅', () => {
    const ownerId = 'test_actor';
    sm.addEffectBySkill(ownerId, 'バイキルト'); // duration: 3
    ...
    expect(removed[0].effect).toBe('atk_up');
    effects = sm.getEffects(ownerId);
  ...
  test('removeEffect で効果を削除', () => {
    ...
    const removed = sm.removeEffect(ownerId, 'atk_up');
    expect(removed.length).toBe(1);
    expect(removed[0].effect).toBe('atk_up');
